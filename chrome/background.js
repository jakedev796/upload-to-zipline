...
async function createContextMenu() {
    ...
    const { shortenEnabled } = await chrome.storage.sync.get(['shortenEnabled']);
    if (shortenEnabled) {
        chrome.contextMenus.create({
            id: "shortenURL",
    ...
    if (info.menuItemId === "uploadToZipline") {
        ...
        try {
            const { requestURL, authToken, expiryEnabled, expiryTime, maxViewsEnabled, maxViews } =
                await chrome.storage.sync.get(['requestURL', 'authToken', 'expiryEnabled', 'expiryTime', 'maxViewsEnabled', 'maxViews']);
